...
db_fields = ['sample_id', 'imported_on', 'resolved_on', 'patient_id',
             'facility_code', 'collected_on',
             'received_on', 'processed_on', 'result', 'result_detail',
             'birthdate',
             'child_age_unit', 'health_worker', 'health_worker_title',
             'sync_status', 'verified', 'care_clinic_no', 'phone', 'sex']
...
def dbconn(db):
    """return a database connected to the production (lab access) or staging
     (UNICEF sqlite) databases"""

    if db == 'prod':
        return MySQLdb.connect(host='127.0.0.1', port=3306, user='rapidsms',
                               passwd='rapidsms-results',
                               db='rapidsms_results')
    elif db == 'lims':
        return MySQLdb.connect(host='127.0.0.1', port=3306, user='mwana',
                               passwd='mwana-labs', db="eid_central")
    elif db == 'staging':
        return MySQLdb.connect(host='127.0.0.1', port=3306, user='rapidsms',
                               passwd='rapidsms-results',
                               db='rapidsms_results')
...
def pkey_fetch(curs, query, args=()):
    """query the primary key column of a table and return the results as a
    set of ids"""
...
def facilities_where_clause():
    """ensures that only the clinics specified in the config file are
    entered into the staging database"""
    if config.clinics:
...
def archive_old_samples(lookback):
    """pre-populate very old samples in the prod db into staging, so they don't
show up in 'new record' queries"""
    conn = dbconn('lims')
    curs = conn.cursor()
    sql = "select %s, %s from %s where %s < '%s'" % (config.lims_db_id_column,
                                                     config.lims_db_lab_column,
    ...
    archive_ids = set([config.get_unique_id(rec[0], rec[1]) for rec in curs.fetchall()])
    ...
    for id in archive_ids:
        curs.execute(
            "insert into samples (sample_id, sync_status) values (%s, 'historical')",
            [id])
    conn.commit()
    curs.close()
    conn.close()
...
def get_ids(db, col, table, normfunc=identity):
    ...
    if db == 'prod' and facilities:
        ...
        # results for the proper facilities are entered into the
        # staging database
...
def check_new_records():
    """poll if any new records have appeared in the production db
    (since last time it was synced to staging)"""
    ...
    if len(deleted_ids) > 0:
        log.warning('records deleted from lab database! (%s)' % ', '.join(
            sorted(list(deleted_ids))))
...
def tx(val, func=identity, nullval=None):
    ...
    return func(val) if val is not None else nullval


def query_sample(sample_id, conn=None):
    """execute a query to retrieve a single sample row
    from the production db"""
    one_time_conn = (conn is None)
    if one_time_conn:
        conn = dbconn('prod')
...
def read_sample_record(sample_id, conn=None):
    ...
    if sample_row[5] is not None and sample_row[
        5].lower() not in config.result_map:
...
def get_update_ids(deleted_ids):
    """get the ids of sample records that still lie within the various
    'update listening' windows"""
    conn = dbconn('staging')
    curs = conn.cursor()
    ...
    # ids for records where result reported;
    # still listening for any changes to record
    update_window_ids = sample_window(
      curs,
      "in ('positive', 'negative', 'rejected')",
      'resolved_on', config.result_window)

    # ids for records where status indetermined;
    # waiting for followup/resolution updates
    incomplete_window_ids = sample_window(
      curs,
      "in ('indeterminate', 'inconsistent')",
      'resolved_on',
      config.unresolved_window)

    # ids for records still awaiting test result
    testing_window_ids = sample_window(curs, 'is null', 'imported_on',
                                       config.testing_window)
...
def query_prod_records():
    """get all records of interest from the lab database (new records and
    records for which still listening for updates)"""
    (new_ids, deleted_ids) = check_new_records()
    (update_window_ids, incomplete_window_ids,
     testing_window_ids) = get_update_ids(deleted_ids)
    ids_of_interest = new_ids | update_window_ids | incomplete_window_ids | \
                      testing_window_ids

    log.info(
        'querying records of interest from lab: %d total%s; %d new; %d resolved; %d in limbo; %d untested' %
        (len(ids_of_interest), (' (+ %d to delete)' % len(deleted_ids)) if len(
            deleted_ids) > 0 else '',
         len(new_ids), len(update_window_ids), len(incomplete_window_ids),
         len(testing_window_ids)))
...
def pull_records():
    ...
    try:
        ...
        log.exception(
            'error accessing lab database (read-only); staging database not touched')
    ...
    try:
        ...
        if config.clinics:
            log.info(
                'staging db: added %d (%d) new records, updated %d (%d) existing records, deleted %d records' %
                (newcount, newcount_filt, updatecount, updatecount_filt,
                 len(deleted_ids)))
        else:
            log.info(
                'staging db: added %d new records, updated %d existing records, deleted %d records' % (
                    newcount, updatecount, len(deleted_ids)))
    except:
        log.exception(
            'error syncing data to staging database; attemping to rollback')
        conn.rollback()
        raise RuntimeError('caught')
    finally:
        curs.close()
...
def process_record(record, source, curs):
    ...
    filt = record[
               'facility_code'] in config.clinics if config.clinics else True

    if source == 'new':
        record['imported_on'] = date.today()
        record['sync_status'] = 'new'
        record['resolved_on'] = date.today() if record[
        ...
        if len(changed_fields) > 0:
            log.info('record [%s] updated: %s' % (
                record['sample_id'], ', '.join(sorted(list(changed_fields)))))
            ...
            # if 'result' changed from a non-definitive result to a
            # definitive result, reset the 'resolved_on' counter
            if 'result' in changed_fields and update_resolved_date(
                    existing_record['result'], record['result']):
...
def update_resolved_date(old_result, new_result):
    """whether to reset the 'resolved on' counter based on
    change in result status"""
    if old_result == None:  # new result must be different, therefore not None
        return True
    elif old_result in ('indeterminate', 'inconsistent') and new_result in (
...
def add_record(record, curs):
  ...
  ''' % (', '.join(db_fields), ', '.join(['%s' for f in db_fields])),
                 [record[f] for f in db_fields])


def update_record(record, curs):
    """update a record in staging; delete then re-add, because the sql is
easier"""
...
def read_logs():
    ...
    if logline != '':
        yield parse_log_line(
            '2000-01-01 00:00:00,000;WARNING;incomplete log entry:[%s]' % logline)


def get_unsynced_logs():
    """return a list of all log entries that have not been synced
    to the server"""
    ...
    if not reached_coll_point:
        logs.append(parse_log_line(
            '2000-01-01 00:00:00,000;WARNING;reached end of logs'))
...
def get_unsynced_records():
    ...
    conn = dbconn('staging')
    curs = conn.cursor()
    unsynced_ids = pkey_fetch(
      curs, "select sample_id from samples where sync_status "
      "in ('new', 'update')")
    curs.close()
...
def retry_task(task, retry_sched):
    ...
    the task is encapsulated in the 'task' object; see the *Task classes.
    the max number of retries and delays between them is determined by the
    retry_sched param, a list of retry_delays in seconds"""
    success = False
    tries = 0
...
class DBSyncTask:
    ...
    def hook_fail_retry(self, tries, total_tries, retry_wait):
        log.info(
            'db sync attempt %d of %d failed; trying again in %d minutes' % (
                tries, total_tries, retry_wait / 60))
...
class GetUnsyncedRecordsTask:
    """retryable task for pulling the set of records to sync from the
    staging database"""
    ...
    def do(self):
        try:
            ...
            log.exception(
                'could not read records to sync from staging database')
            return False
    ...
    def hook_fail(self, total_tries):
        log.info(
            'could not read records to sync; no records will be sent in this payload')
...
def condense_record(record):
    """trim unnecessary fields and shorten field names to condense sample
    record size for transmission"""
...
def aggregate_submit_data():
    """collect all the different types of data to be sent to rapidsms
    (sample records, log entries), and return as a unified list of datums"""
    ...
    log.info('%d sample records and %d log entries to send' % (
        len(sync_records), len(logs)))
    return interlace_data(sync_records, logs)


def interlace_data(records, logs):
    """we attempt to chunk the data into many small submissions to help
    mitigate the effect of flaky connections; this function attempts to
    optimize the distribution of our data records in case only a few POSTs
    get through"""

    # send logs first in reverse-chron order
    # (logs are passed in already sorted)
    for log in logs:
        yield ('log', log)
...
def chunk_submissions(data_stream):
    """turn the stream of data objects into transmission chunks of 
    (approximate) max size"""
    chunk_size_limit = config.transport_chunk / (
        config.compression_factor if config.send_compressed else 1.)
...
class Payload:
    """a container class that represents a json payload and the sample ids
    of the records contained within it"""
...
def connection(payload):
    """undertake the actual http(s) connection; report success or failure with
message"""
    try:
        authinfo = urllib2.HTTPBasicAuthHandler()
        urlinfo = urlparse(config.submit_url)
        authinfo.add_password(uri=urlinfo[0] + '://' + urlinfo[1],
                              **config.auth_params)
        ...
        try:
            f = urllib2.urlopen(
                urllib2.Request(config.submit_url, payload.json,
                                headers=headers))
            response = f.read()
            code = f.code
...
class UpdateSyncFlagTask:
    """retryable task to update the 'sync' flag of records that
        ...
        try:
            try:
                ...
                for id in self.payload.record_ids:
                    curs.execute(
                        "update samples set sync_status = 'synced' where sample_id = %s",
                        [id])
                conn.commit()
                return True
            except:
    ...
    def hook_fail(self, total_tries):
        log.warning(
            'successfully sent records, but failed to update sync flag; records will be resent in next batch')
...
def trunc(text):
    """error message can come from arbitrary http response,
    so check that length is reasonable"""
    limit = 300
...
def send_payload(payload):
    ...
    if success:
        update_sync_flag(
            payload)  # even if this fails, we don't want to count it as
                      # a 'send' failure, so we still return success = True
    else:
...
class SendAllTask:
    """task manager for sending N payloads,
    allowing a certain number of retries"""
    ...
    def do(self):
        while self.i < len(self.payloads):
            ...
            if success:
                self.i += 1
                log.debug(
                    'sent payload %d of %d' % (self.i, len(self.payloads)))
            else:
                return False
        return True
    ...
    def hook_fail(self, total_tries):
        log.warning(
            'too many failed send attempts; aborting send; %d of %d payloads successfully transmitted' % (
                self.i, len(self.payloads)))

    def hook_fail_retry(self, tries, total_tries, retry_wait):
        log.warning(
            'failed send on payload %d of %d; %d tries left; resuming in %d seconds' % (
                self.i + 1, len(self.payloads), total_tries - tries,
                retry_wait))
...
def send_data():
    ...
    payloads = [Payload(chunk=chunk, id='%d/%d' % (i + 1, len(chunks))) for
                (i, chunk) in enumerate(chunks)]
    if len(
            payloads) == 0:  # nb: should rarely, if ever, happen; there will always be a few new log messages
        payloads = [Payload()]
        log.info('no data to send; sending ping message only')
    else:
        log.info('%d payloads to send (%d bytes%s)' % (
            len(payloads), sum(len(p.json) for p in payloads),
...
def main():
    ...
    try:
        ...
        log.exception(
            'caught exception in config.bootstrap; continuing anyways')
    try:
        sync_databases()
        send_data()
    except:
    ...
    try:
        ...
        log.exception(
            'caught exception in config.teardown; continuing anyways')
    log.info('extract/sync task complete')


class SingletonTask:
    """wrapper class to ensure that a function/task has only one executing
    instance at any given time; accomplishes this in a somewhat ghetto manner
    by polling a shared lockfile"""

    def __init__(self, task, lockfile, poll_freq=20, max_runtime=None,
                 name=''):
        self.task = task
        ...
        if self.acquire_lock():
            ...
            while thr.is_alive():
                if self.max_runtime != None and runtime > self.max_runtime:
                    log.warning((
                      'task [%s] exceeded its max allowed runtime of %ds; runlock is being released; ' +
                      'assume task is hung/stalled, but may still be running!') % (
                        self.name, self.max_runtime))
                    break
            ...
            log.info(
                'unable to acquire lock for task [%s]; not running...' % self.name)
    ...
    def read_lockfile(self):
        try:
            ...
            log.exception(
                'could not read lockfile [%s - %s]; proceeding as if unlocked...' % (
                    self.name, self.lockfile))
            return None
    ...
    def refresh_lock(self):
        try:
            ...
            log.exception(
                'cannot update lockfile [%s - %s]; singleton guarantees no longer valid, but not much we can do; proceeding...' % (
                    self.name, self.lockfile))
    ...
    def monitor_lockfile(self, val):
        for i in range(0, 6):
            ...
            if new_val != None and val != new_val:
                return False
        log.info(
            'existing lockfile [%s - %s] appears to be from a defunct process; proceeding as if unlocked...' % (
                self.name, self.lockfile))
        return True
    ...
    def clear_lock(self):
        try:
            ...
            log.exception(
                'could not clear lockfile [%s - %s]; task is now defunct' % (
                    self.name, self.lockfile))
...
def fire_task():
    ...
    try:
        ...
        SingletonTask(main, config.task_lock, name='extract/sync',
                      max_runtime=7200).start()
...
def is_hit(sched_time, time_a, time_b):
    """return true if the sched_time falls within the bracket times time_a and
time_b"""
    if time_a < time_b:
        return time_a <= sched_time < time_b
    else:  # midnight rollover
        return time_a <= sched_time or sched_time < time_b


def daemon_loop(times, last_ping):
    """core daemon loop that waits for scheduling events
    and launches the tasks"""
    try:
        log.info('daemon booted; ' + (
            'previous instance appears to have terminated around %s' % last_ping
            if last_ping != None
    ...
    while True:
        try:
            ...
            # if new time is before old time, or new time is significantly
            # later than old time, assume the system
            # clock changed or something else weird happened; don't fire tasks
            if new_inst < inst or fdelta(new_inst - inst) > 300:
                log.info('time discrepancy detected: ' + ' -> '.join(
                    t.strftime('%Y-%m-%d %H:%M:%S') for t in [inst, new_inst]))
            else:
                time_a = inst.time()
                time_b = new_inst.time()
                sched_hit = any(
                    is_hit(sched_time, time_a, time_b) for sched_time in times)
            ...
            if fault_count == max_faults:
                log.info(
                    'too many faults (%d) in daemon loop; shutting down as a precaution...' % max_faults)
                return
...
def daemon():
    """ENTRY POINT: background daemon that runs the extract/sync task
    ...
    try:
        ...
        if len(times) == 0:
            ...
            log.info('scheduled to run at: %s' % ', '.join(
                t.strftime('%H:%M') for t in sorted(times)))

        if config.clinics:
            log.info('filtering by %d clinics: %s' % (
                len(config.clinics), ', '.join(config.clinics)))
        else:
            log.info('all clinics enabled')

        last_ping = SingletonTask(None, config.daemon_lock).read_lockfile()
        SingletonTask(lambda: daemon_loop(times, last_ping),
                      config.daemon_lock, name='daemon').start()
    except:
...
def test_script():
    init()
    log.info('initialized database now running test extraction')
    # set lookback to none
    config.init_lookback = None
    log.info('running test extraction, change the submit url on success.')
    main()
