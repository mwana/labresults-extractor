...
# temporary path defined in __init__
prod_db_path = os.path.join(base_path, 'prod_results.db3')
...
# the name of the column in prod_db_table containing the lab-based ID of the
#  record
...
lims_db_lab_column = 'labtestedin'
...
result_window = 365  # number of days to listen for further changes after a
# definitive result has been reported
unresolved_window = 365  # number of days to listen for further changes
# after a non-definitive result has been
# reported (indeterminate, inconsistent)
testing_window = 365  # number of days after a requisition forms has been
# entered into the system to wait for a
# result to be reported
init_lookback = "2014-08-01"  # when initializing the system, from when
# to send the data, "YYYY-mm-dd",
# results for (everything before that is 'archived').  if None, no archiving
#  is done.
transport_chunk = 5000  # maximum size per POST to rapidsms server (bytes) (
# approximate)
...
def get_unique_id(sample_id, lab):
    # global source_id
    sample_id = str(sample_id) + labs_map[str(lab)]
    return sample_id
...
def bootstrap(log):
    """creates a temporary mysql-based production table to speed prod db
    queries"""
    log.debug('copy records from mysql into temp prod db table')
    # connect to MySQL
    eid_db = MySQLdb.connect(host='127.0.0.1', port=3306, user='mwana',
                             passwd='mwana-labs', db="eid_central")
    eid_curs = eid_db.cursor()
    prod_db = MySQLdb.connect(host='127.0.0.1', port=3306, user='rapidsms',
    ...
    while row:
        row = list(row)
        row[0] = get_unique_id(row[0], row[18])
        paramspec = " ,".join(["%s"] * len(row))
        insert_sql = 'INSERT INTO {} ({}) VALUES ({});'.format(prod_db_table,
                                                               cols, paramspec)
